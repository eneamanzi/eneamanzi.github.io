...
Caso diverso dagli altri: qui la funzionalità **verrà usata** (pubblicazioni future), quindi niente esclusione dalla build — solo "in pausa", pronta a riprendere senza dover rimontare nulla.

- **Cosa**: voce "publications" tolta dalla navbar (pagina non esclusa dalla build: è vuota, non ci sono contenuti stantii/sensibili da nascondere come nel caso del blog, quindi restare raggiungibile-ma-vuota non è un problema).
  - File: `_pages/publications.md` → `nav: true` → `nav: false`
  - Per riattivare: rimetti `nav: true` (basta questo, nessun'altra modifica necessaria) quando aggiungi la prima pubblicazione vera.

- **Cosa**: bibliografia placeholder neutralizzata, non cancellata.
  - File: `_bibliography/papers.bib`
  - Modifica: tutte le 9 voci di esempio (Einstein) sono commentate riga per riga con `%` (commento di riga standard BibTeX). Aggiunta anche una nota in testa al file che spiega la cosa.
  - Include anche l'esempio del campo `selected = {true}` (dentro `PhysRev.47.777`), ora inerte perché tutta la voce è commentata — resta come riferimento su come marcare in futuro un tuo paper come "in evidenza" nella pagina about (`selected_papers: true` in `about.md` non è stato toccato, non fa nulla finché non c'è una voce reale con `selected = {true}`).
  - Per aggiungere una pubblicazione vera: basta scrivere una normale voce `@article{...}` / `@book{...}` in qualsiasi punto del file (jekyll-scholar la trova da sola), non serve toccare le righe commentate.
